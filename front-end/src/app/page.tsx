...
export default async function HomePage({
    ...
    let season;
    if (searchParams && searchParams.season) {
        if (searchParams.season == '23-24') {
            season = new Season(2023, 2024);
        } else if (searchParams.season == '24-25') {
            season = new Season(2024, 2025);
        } else {
            throw new Error(`Unknown season: ${searchParams.season}`);
        }
    } else {
        season = new Season(2023, 2024);
    }
    ...
    return (
        <main className="flex flex-col gap-4 px-6">
            <div className="flex flex-row items-center justify-between">
                ...
                <SeasonDropdown selectedSeason={{...new Season(2023, 2024)}} seasons={[{...new Season(2023, 2024)}, {...new Season(2024, 2025)}]} />
            </div>
